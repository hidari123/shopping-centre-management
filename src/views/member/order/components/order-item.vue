      ...
      <div class="order-item">
        ...
        <div class="body">
          ...
          <div class="column action">
            ...
            <!-- 已完成：查看详情，再次购买，申请售后 -->
            <!-- 已取消：查看详情 -->
            <XtxButton @click="$router.push(`/member/pay?id=${order.id}`)" v-if="order.orderState===1 && order.countdown > -1" type="primary" size="small">立即付款</XtxButton>
            <XtxButton @click="$emit('on-confirm', order)" v-if="order.orderState===3" type="primary" size="small">确认收货</XtxButton>
            <p><a  @click="$router.push(`/member/order/${order.id}`)">查看详情</a></p>
            <p @click="$emit('on-cancel', order)" v-if="order.orderState===1 && order.countdown > -1"><a href="javascript:;">取消订单</a></p>
